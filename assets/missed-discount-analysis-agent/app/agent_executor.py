...
from tools.email_notification import send_email_notification as _send_email


@lc_tool
def send_email_notification(recipients: list[str], invoices: list[dict], top_n: int = 10) -> dict:
    """Send an email summary of the top-N at-risk/lost invoices to the specified recipients.

    Args:
        recipients: List of recipient email addresses provided by the user.
        invoices:   Enriched invoice list (output of discount analysis). Only at-risk and lost invoices are emailed.
        top_n:      Maximum number of invoices to include in the summary (default 10).

    Returns:
        Dict with keys: sent (bool), recipients (list), invoice_count (int), and optionally reason (str).
    """
    return _send_email(recipients=recipients, invoices=invoices, top_n=top_n)
...
class AgentExecutor(A2AAgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        tools = [*tools, *self.skill_tools, send_email_notification]
